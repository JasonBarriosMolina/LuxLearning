...
[{"title":"Título","content":"<h3>subtítulo</h3><p>Contenido HTML</p>","points":["punto 1 (texto plano, sin markdown ni HTML)","punto 2","punto 3"],"tip":"consejo práctico","type":"video|text","duration":"5 min|${textDuration}"}]`;
    ...
    const generateModuleClass = async (moduleIdx: number): Promise<void> => {
      ...
      try {
        ...
        // Opening greeting added back (Trello DmPpbrff, 2026-09-04 — Mack: "Lux Mentor...
        // debe de ingresar saludando y agradeciendo su permanencia en el curso, y que está
        // emocionado por enseñar más contenido"), scoped as ONE bounded sentence so it
        // doesn't reopen the chatbot-filler problem the 2026-08-30 fix removed — a warm,
        // professional welcome, not "¡Hola! ¡Buena pregunta!" chattiness throughout.
        const classPrompt = isBlEN
          ? `Generate a Lux Mentor class for module "${mod.title}". Professional, formal educational tone — no emojis, no chatbot-style filler ("Great question!", etc) anywhere in the body. JSON: {"vapiPrompt":"<prompt for a live Q&A voice tutor — pose guiding questions about the module, 100 words max>","lessonScript":"<START with ONE sentence: warmly greet the student, thank them for staying in the course, and say you're excited to teach more. Then the class outline as clear bullet points (- item) covering 3 key topics and activities, 400-450 words TOTAL including that opening sentence (this is a narrated 'clase magistral', not a summary — this is close to the maximum a single narration request can hold), written to be read aloud by a narrator, with natural pauses between points>","closingScript":"<~90-110 word spoken closing: thank the student for the time spent together, say it was a pleasure talking with them, that you hope everything covered helps them, and that the class is closing now. Briefly recap the key topics covered in this module. End by mentioning that the full transcript and interaction from today's class will be available shortly for them to revisit whenever they want. Warm, one paragraph, natural spoken closing — not a bullet list>"}`
          : `Genera una Clase Magistral Lux Mentor para el módulo "${mod.title}". Tono profesional y educativo formal — sin emojis, sin muletillas de chatbot ("¡Buena pregunta!", etc) en ningún otro lugar del cuerpo. JSON: {"vapiPrompt":"<prompt para un tutor de voz en vivo — plantea preguntas guía sobre el módulo, máx 100 palabras>","lessonScript":"<EMPIEZA con UNA oración: saluda cálidamente al estudiante, agradécele por continuar en el curso, y di que estás emocionado por enseñar más contenido. Luego el esquema de clase en viñetas claras (- item) cubriendo 3 temas clave y actividades, 400-450 palabras EN TOTAL incluyendo esa oración inicial (esto es una clase magistral narrada, no un resumen — este es cerca del máximo que una sola narración puede contener), escrito para ser leído en voz alta por un narrador, con pausas naturales entre puntos>","closingScript":"<cierre hablado de ~90-110 palabras: agradece al estudiante por el tiempo compartido, di que fue un placer conversar con él/ella, que esperas que todo lo visto le sea de ayuda, y que la clase se está cerrando en este momento. Resume brevemente los temas clave vistos en este módulo. Termina mencionando que la transcripción completa y la interacción de la clase de hoy estarán disponibles en breve para que la revisite cuando quiera. Cálido, un solo párrafo, cierre hablado natural — no una lista de viñetas>"}`;
