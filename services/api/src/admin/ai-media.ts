// AI course generation and media (images, covers, stock photos) domain handler for lux-admin.
...
// Stricter Bedrock caller used in ai-generate worker — applies fixJsonControlChars before parsing.
async function bedrockJSON(prompt: string, maxTokens = 2000): Promise<any> {
  ...
  try { return JSON.parse(jsonStr); } catch {
    try { return JSON.parse(jsonrepair(jsonStr)); } catch { return {}; }
  }
}

export async function handleAIMedia(ctx: AdminCtx): Promise<any | null> {
...
Responde ÚNICAMENTE con un array JSON de strings. Ejemplo: ["liderazgo","comunicación","gestión"]`;
  ...
  if (generateCoverMatch && method === 'POST') {
    ...
    const imageUrl = await generateLessonImage(course.title, '', 0, { promptText: promptText.trim(), style, s3Prefix: 'covers' } as any);
  ...
  if (path === '/admin/generate-image' && method === 'POST') {
    ...
    const { promptText, style } = body as { promptText?: string; style?: string };
    if (!promptText?.trim()) return badRequest('promptText es requerido');
    const imageUrl = await generateLessonImage('', '', 0, { promptText: promptText.trim(), style, s3Prefix: 'editor' } as any);
    if (!imageUrl) return badRequest('No se pudo generar la imagen. Intenta de nuevo.');
    return ok({ imageUrl });
  }
  ...
  if (path === '/admin/stock-photos' && method === 'GET') {
    ...
    const photos = (data.results ?? []).map((p: any) => ({ id: p.id, thumb: p.urls.small, full: p.urls.regular, author: p.user.name, authorUrl: p.user.links.html }));
