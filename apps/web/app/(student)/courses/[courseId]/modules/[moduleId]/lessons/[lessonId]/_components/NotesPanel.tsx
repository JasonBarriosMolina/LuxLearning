...
//
// Redesigned as a floating panel (Trello DmPpbrff, 2026-09-05 — Mack: "no quiero
// que mis notas sea siempre un cuadro estático abajo ... quiero que sea como una
// burbuja ... tipo chatbot, en el cual yo pueda darle clic y que se abra"): was a
// static inline card in the page flow; now a fixed overlay whose open/close state
// and floating trigger button (NotebookPen icon, not an emoji) live in page.tsx —
// same pattern already used there for the Mentor chat and forum overlays (this
// component just renders its own panel chrome; the parent decides whether to
// mount it at all).
import { useState, useEffect, useCallback } from 'react';
import { Sparkles, Trash2, Search, Tag as TagIcon, Loader2, NotebookPen } from 'lucide-react';
...
export function NotesPanel({ contextType, contextId, lessonTitle, highlightsForSummary = [] }: Props) {
  ...
  return (
    <div className="fixed bottom-24 right-36 z-50 w-80 h-[70vh] flex flex-col bg-white dark:bg-[#1A1A2E] rounded-2xl shadow-2xl border border-border animate-fade-in overflow-hidden">
      {/* Header — matches the Mentor chat / forum overlays' convention exactly */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-gradient-to-r from-purple-500 to-fuchsia-500 shrink-0">
        <div className="flex items-center gap-2">
          <NotebookPen className="w-4 h-4 text-white" />
          <span className="text-white font-semibold text-sm">{tp.title}</span>
        </div>
        <button
                ...
                {filtered.map((n) => (
                  <div key={n.noteId} className="p-3 rounded-xl border border-border space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-sm text-charcoal dark:text-gray-200 whitespace-pre-wrap flex-1">{n.text}</p>
