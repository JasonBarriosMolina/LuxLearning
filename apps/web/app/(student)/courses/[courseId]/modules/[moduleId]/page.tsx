...
export default function ModulePage() {
  ...
  if (!module) {
    return (
      <div className="max-w-3xl mx-auto card text-center py-16">
        <p className="font-heading font-bold text-xl text-charcoal">Módulo no encontrado</p>
      </div>
    );
  }
  ...
  const getModuleStatus = () => {
    if (reflectionStatus === 'APPROVED') return { label: 'Completado', variant: 'success' as const };
    if (reflectionStatus === 'PENDING_EVAL') return { label: 'Reflexión en revisión', variant: 'pending' as const };
    if (reflectionStatus === 'PENDING_AI') return { label: 'Procesando reflexión', variant: 'info' as const };
    if (reflectionStatus === 'REJECTED') return { label: 'Reflexión rechazada', variant: 'error' as const };
    if (module.quizPassed) return { label: 'Quiz aprobado', variant: 'success' as const };
    if (allLessonsDone) return { label: 'Listo para quiz', variant: 'warning' as const };
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="card">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <p className="text-xs text-gray-400 font-semibold mb-1">MÓDULO {module.order}</p>
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
          ...
          <span className="flex items-center gap-1">
            <ClipboardCheck className="w-4 h-4" /> Nota mínima: {module.passingScore}%
          </span>
        </div>

        <ProgressBar value={progress} label={`${completedLessons.length} de ${totalLessons} lecciones`} showPercent />
      </div>
      ...
      <div className={`card ${!allLessonsDone ? 'opacity-60' : ''}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-charcoal text-sm">Quiz del módulo</p>
              <p className="text-xs text-gray-500">
                {module.quizPassed
                  ? 'Aprobado'
                  : allLessonsDone
                  ? 'Disponible — completa el quiz para continuar'
          ...
          {allLessonsDone && !module.quizPassed && (
            <Link href={`/courses/${courseId}/modules/${moduleId}/quiz`}>
              <Button size="sm">Tomar quiz</Button>
            </Link>
          )}
          {module.quizPassed && (
      ...
      <div className={`card ${!module.quizPassed ? 'opacity-60' : ''}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-charcoal text-sm">Reflexión del módulo</p>
              <p className="text-xs text-gray-500">
                {reflectionStatus
                  ? { PENDING_AI: 'Procesando con IA...', PENDING_EVAL: 'En revisión por evaluador', APPROVED: 'Aprobada — módulo completado', REJECTED: 'Rechazada — puedes reescribirla' }[reflectionStatus]
                  : module.quizPassed
                  ? 'Escribe tu reflexión para completar el módulo'
                  : 'Aprueba el quiz primero'}
              </p>
            </div>
          </div>
          {module.quizPassed && (!reflectionStatus || reflectionStatus === 'REJECTED') && (
            <Link href={`/courses/${courseId}/modules/${moduleId}/reflection`}>
              <Button size="sm" variant={reflectionStatus === 'REJECTED' ? 'secondary' : 'primary'}>
                {reflectionStatus === 'REJECTED' ? 'Reescribir' : 'Escribir'}
              </Button>
            </Link>
          )}
