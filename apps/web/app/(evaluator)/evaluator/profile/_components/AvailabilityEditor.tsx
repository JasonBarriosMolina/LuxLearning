...
export function AvailabilityEditor({ username }: { username: string }) {
  ...
  const addBlock = () => setBlocks((b) => [...b, { dayOfWeek: 1, startTime: '08:00', endTime: '12:00' }]);
    ...
    <div className="card">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">Tus bloques libres entre semana (el sábado presencial usa un horario institucional fijo)</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {blocks.map((b, i) => (
          <div key={i} className="flex items-center gap-2 p-3 bg-surface rounded-xl">
            ...
            <input type="time" value={b.startTime} onChange={(e) => updateBlock(i, { startTime: e.target.value })} className="input-field text-sm py-1.5 w-28" />
